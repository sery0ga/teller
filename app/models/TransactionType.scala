...
import models.database.{ BookingEntriesOverflow, BookingEntries, TransactionTypes }
...
object TransactionType {
  ...
  def delete(id: Long): Unit = withSession { implicit session ⇒
    BookingEntriesOverflow.filter(_.transactionTypeId === id).map(_.transactionTypeId).update(None)
